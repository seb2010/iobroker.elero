...
class EleroTransmitters{
    ...
    discover(){
        ...
        this._adapter.log.debug('Discovvery: Discovering Transmitters...');
        var i = 0;
		serialport.list(function (err, ports) {
            ports.forEach(function (port) {
                i++;
                this._adapter.log.debug(port.path);
                this._adapter.log.debug(port.serialNumber);
                this._adapter.log.debug(port.manufacturer);
                if (DEFAULT_BRAND in port.manufacturer && DEFAULT_PRODUCT in port.product) {
                    this._adapter.log.debug("Elero - an Elero Transmitter Stick is found on port: '" + port.path + "'" + " with serial number: '" + port.serialNumber + "'");
                    ...
                    this._adapter.log.debug("Initializing Transmitter '" + port.serialNumber + "' at Port: '" + port.name + "'");
                    var elero_transmitter = new EleroTransmitter(this._adapter, port, baudrate, bytesize, parity, stopbits);

                    if (elero_transmitter.get_transmitter_state()) {
                        this._adapter.log.debug("Transmitter '" + port.serialNumber + "' initialized");
                        if (!(String(port.serialNumber) in this.transmitters)) {
                            this.transmitters[port.serialNumber] = elero_transmitter;
                        } else {
                            this._adapter.log.debug("Elero - '" + port.serialNumber + "' transmitter is already added!");
                        }
                    }
                }
            });
